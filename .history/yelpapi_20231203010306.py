...
def search_yelp(api_key, search_term, userPrice, latitude, longitude, radius=1000):
    ...
    hours_list_one = [] #empty list
    hours_list_two = [] #empty list
    hours_list_three = [] #empty list
    for i in list_one:
        hours_list_one.append(hours_search(api_key, i["id"]))
    for i in list_two:
        hours_list_two.append(hours_search(api_key, i["id"]))
    for i in list_three:
        hours_list_three.append(hours_search(api_key, i["id"]))
    
    # now can filter out the results that do not have hours of operation
    print(len(list_one))
    for i in hours_list_one:
        if i[0]["day"] == "N/A":
            list_one.remove(i)
    for i in hours_list_two:
        if i[0]["day"] == "N/A":
            list_two.remove(i)
    for i in hours_list_three:
        if i[0]["day"] == "N/A":
            list_three.remove(i)
    print(len(list_one))
